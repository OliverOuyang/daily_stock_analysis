...
{action_text}
"""
            # 持仓场景下，强制输出可执行的加减仓计划，并保持策略连贯性
            if profile.get('status') == 'holding':
                prompt += """

### 持仓股强制规则（必须执行）
1. 必须给出**明确减仓点**：包含具体价格 + 建议减仓比例（示例：`35.00 元减仓 50%`）。
2. 必须给出**明确补仓点**：包含具体价格 + 建议补仓比例（示例：`30.00 元补仓 20%`）。
3. 必须保持**策略连贯性**：结合最近操作记录（action_history）和当前剩余仓位，不能给出与已执行动作冲突的建议。
4. 若关键数据缺失，必须显式说明缺失原因，但仍需给出保守的仓位动作框架（例如“反弹减仓、回踩小幅补仓”）。
"""
