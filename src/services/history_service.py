...
class HistoryService:
    ...
    def _record_to_detail_dict(self, record) -> Dict[str, Any]:
        ...
        return {
            ...
            "ideal_buy_value": float(record.ideal_buy) if record.ideal_buy is not None else None,
            "secondary_buy": str(record.secondary_buy) if record.secondary_buy else None,
            "secondary_buy_value": float(record.secondary_buy) if record.secondary_buy is not None else None,
            "stop_loss": str(record.stop_loss) if record.stop_loss else None,
            "stop_loss_value": float(record.stop_loss) if record.stop_loss is not None else None,
            "take_profit": str(record.take_profit) if record.take_profit else None,
            "take_profit_value": float(record.take_profit) if record.take_profit is not None else None,
