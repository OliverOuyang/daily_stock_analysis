...
_THINKING_TOOL_LABELS: Dict[str, str] = {
    ...
    "get_market_climate": "宏观环境获取",
    "get_money_flow": "资金流向获取",
    "get_lhb_insight": "龙虎榜席位分析",
}
...
**第一阶段 · 宏观与大势**（首先执行）
- `get_market_climate` 获取两市成交额、汇率、国债收益率
- `get_money_flow` 获取北向资金流向

**第二阶段 · 行情与K线**（等第一阶段完成后执行）
- `get_realtime_quote` 获取实时行情
- `get_daily_history` 获取历史K线

**第三阶段 · 技术与席位**（等第二阶段结果返回后执行）
- `analyze_trend` 获取技术指标
- `get_chip_distribution` 获取筹码分布
- `get_lhb_insight` 获取龙虎榜席位动向（若有）

**第四阶段 · 情报搜索**（等前三阶段完成后执行）
